...
export class DocumentUploadPage implements OnInit {
  tareas: Tarea[] = [
    ...
    { id: 5, nombre: 'Comprobante de domicilio', status: false },
  ...
  async openModal(tarea: Tarea) {
    ...
    switch (tarea.nombre) {
      ...
      case 'Comprobante de domicilio':
        component = ComprobanteDomicilioModalComponent;
        break;
      case 'Contrato':
  ...
  obtenerEstadoArchivos() {
    ...
    this.http.get<any[]>(`https://siinad.mx/php/getDocumentStatus.php?companyId=${companyId}&userId=${userId}`)
      .subscribe(response => {
